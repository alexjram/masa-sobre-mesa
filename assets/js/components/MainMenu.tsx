...
export default function MainMenu({ foodTypes, selectedType, onSelectingType, shouldDecreaseSize }: Props) {
	...
	return (
		<header className="flex-0 bg-white px-4 border">
			<div className={`mx-auto lg:px-[63px] flex justify-center items-center min-h-[40px] w-full relative ${selectedType ? 'lg:justify-between' : ''}`}>
				<div className={`w-[70%] lg:w-[346px] ${!selectedType || !shouldDecreaseSize ? "py-[12%]" : "py-[6%]"} transition-all duration-300 lg:py-10`}>
				...
				{!selectedType && (
					<a href={getOtherLangURL()} className="font-bold use-font uppercase px-5 absolute top-[58px] right-[63px] hidden lg:flex">
						{lang.toLowerCase() === 'en' ? (
							<BritishFlag size={25} />
						) : (
							<SpanishFlag size={25} />
